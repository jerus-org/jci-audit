...
| **The GitHub token** (`verify`'s remote path, `GITHUB_TOKEN` env var only — no CLI flag exists for it, and it is optional) | Trusted, caller-supplied, when present | Held in memory only for the duration of the fetch; when set, authenticates both `pcu-release-assets`' REST calls and, when that pubkey source is tried, the direct `reqwest` fetch of the release tag's `Cargo.toml`. When unset, both fall back to an unauthenticated request instead — the release and its assets are public, and GitHub serves public release-asset downloads and public-repo raw file contents without a token (`ReleaseAssetClient::new_unauthenticated`; jerus-org/jci-audit#103), just at a lower, IP-based rate limit. Never written to a file, logged, or echoed — see T3. |
...
| **T3** | **Credential leakage** — a signing key or token exposed in output, logs, or generated files. | `verify`'s remote fetch path is the only code path that ever reads a credential, and reading one is optional: a GitHub token, read from `GITHUB_TOKEN` **only** — deliberately not a CLI flag, since a secret passed as a command-line argument is visible to any other user on the same machine via `ps`/`/proc/<pid>/cmdline` (`cli.rs::run_verify_remote`). When set, it builds the `Authorization`/bearer header for two things: `pcu-release-assets`' REST calls (`remote::PcuAssetSource`) and, when tried, `remote::ManifestPubkeySource`'s direct `reqwest` fetch; held in memory for the duration of one CLI invocation either way, never written to a file or generated output, and never passed to `tracing::*!` (the module's own log lines name the tag/version/asset/source, not the token). When `GITHUB_TOKEN` is unset, no credential is read at all — both sources construct an unauthenticated client instead (jerus-org/jci-audit#103), since the release and its assets are public. No other code path reads any credential. Satisfies R2. |
...
| **T9** | **Man-in-the-middle on jci-audit's own new network calls** (`verify`'s remote path: the GitHub API via `pcu-release-assets`, and `raw.githubusercontent.com` for the manifest source), **and, separately, a forged release asset (or forged Cargo.toml).** | HTTPS (`reqwest` with `rustls`, TLS certificate verification on by default) mitigates transport-level tampering the same way T5 does for the advisory-db, on both legs. What the minisign check (T6) adds beyond that is **not the same for both pubkey sources, and neither is as independently strong as the ordering alone might suggest** — be honest about each: **`AssetPubkeySource`** has a materially weak property: minisign keys are free to generate (`docs/RELEASING.md` — "an ephemeral key generated per release"), so anyone with **release-asset-upload access alone** (no `github.com`/API compromise required) can mint a fresh keypair offline, sign a forged record with it, and upload record + signature + pubkey together as a self-consistent triple — this source's check alone would accept it. Its real value in that case is narrower: catching an asset **replaced inconsistently** with the others (e.g. only the record swapped, not the signature), not a **coherently forged set**. **`ManifestPubkeySource`** (tried first — currently the only source with real data for any release, since the asset-upload CI step hasn't shipped) reads the pubkey from `Cargo.toml` at the release tag instead. **This is not an independently stronger anchor for jci-audit's own releases.** `docs/RELEASING.md` step 5 and `.circleci/release.yml` show the same `toolkit/release_crate` CI job (context: `release`, `bot-check`, `pcu-app`) both injects the pubkey into `Cargo.toml`/pushes the tag *and* (once #75 phase 2 lands) uploads the record and its signature as release assets — one job, one credential set. Compromising that job's credentials compromises both at once; there is no separately-operated second channel that also has to be broken. It additionally trusts a **mutable** git tag ref (`refs/tags/<tag>`), with no platform-enforced immutability, unlike the published-release-only guarantee GitHub Immutable Releases gives `AssetPubkeySource` — a force-moved tag silently changes what this source fetches. `cli.rs::run_verify_remote` orders the manifest source first because it is **currently the only source with data**, not because of assumed independence; a genuinely stronger, independent anchor would require jci-audit's git-push and release-asset-upload credentials to actually be separate, which they are not today. |
...
- **GitHub API responses** (`verify`'s remote path) are deserialized into typed
  ...
  signing pubkey comes from one of two `PubkeySource`s, tried in order:
  `Cargo.toml` at the release tag, parsed with `toml_edit` and only a known,
  fixed key path (`[package.metadata.binstall.signing].pubkey`) read from it;
  or, as a fallback, the pubkey's own release asset
  (`release-<VERSION>.json.pub`), parsed for the bare key (tolerating either a
  plain key or the full `rsign`-generated pubkey-file format).
...
- **jci-audit's own code now makes network calls, scoped to one path.**
  ...
  the manifest pubkey source is tried (see T9 for why that source is tried
  first). Every other subcommand, and `verify` when a local
