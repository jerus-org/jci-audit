...
- **`--owner`/`--repo`/`--tag-prefix`** (`verify`'s remote-fetch fallback,
  ...
  to `pcu-release-assets`' typed API client. **`--repo-manifest-path`** (opt-in,
  jerus-org/jci-audit#124) is likewise a plain string, interpolated into a
  URL via `format!` — never through a shell, so there is no injection
  surface; a malformed value simply produces a URL that 404s or an API call
  GitHub rejects, not an unintended request elsewhere.
...
- **jci-audit's own code now makes network calls, scoped to one path.**
  ...
  the caller supplies `--repo-manifest-path` (see T9 for why this source is
