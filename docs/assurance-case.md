...
| **The network** (advisory-db fetch, managed by `cargo-deny`; GitHub's REST/GraphQL API and `raw.githubusercontent.com`, used by jci-audit's own code in `verify`'s remote path only) | Untrusted transport | Confidentiality/integrity via TLS on every leg; see T5. Each call also carries the caller's read-only token (T3); the fetched record is additionally authenticated by its minisign signature, independent of transport trust (T6). |
| **The GitHub token** (`verify`'s remote path, `GITHUB_TOKEN` env var only — no CLI flag exists for it) | Trusted, caller-supplied, when present | Authenticates the fetch when set; an unset token falls back to the same requests unauthenticated (jerus-org/jci-audit#103), which works when the named `--owner`/`--repo` release is public — a private repo's release would need one, and the fetch would simply fail without it. Held in memory only for the duration of the fetch; see T3. |
...
| **T3** | **Credential leakage** — a signing key or token exposed in output, logs, or generated files. | `verify`'s remote fetch path is the only code path that ever reads a credential, and even there it's optional, for whichever repository `--owner`/`--repo` name: `GITHUB_TOKEN` **only** — deliberately not a CLI flag, since a secret passed as a command-line argument is visible to any other user on the same machine via `ps`/`/proc/<pid>/cmdline` (`cli.rs::run_verify_remote`). When set, it builds the `Authorization` header for `pcu-release-assets`' REST calls and `remote::ManifestPubkeySource`'s `reqwest` fetch; when unset, both go out unauthenticated instead (jerus-org/jci-audit#103), which works for a public repo's release — a private repo's release would need a token, and the fetch would simply fail without one. Held in memory only, never written to a file or generated output, never logged. No other code path reads any credential. Satisfies R2. |
...
| **T9** | **Man-in-the-middle on jci-audit's own new network calls** (`verify`'s remote path: the GitHub API via `pcu-release-assets`, and `raw.githubusercontent.com` for the manifest source), **and, separately, a forged release asset (or forged Cargo.toml).** | HTTPS (`reqwest` with `rustls`, TLS certificate verification on by default) mitigates transport-level tampering the same way T5 does for the advisory-db, on both legs. What the minisign check (T6) adds beyond that is **not the same for both pubkey sources, and neither is as independently strong as the ordering alone might suggest** — be honest about each: **`AssetPubkeySource`** (tried first, since it depends on nothing about how the release was published) has a materially weak property: minisign keys are free to generate (`docs/RELEASING.md` — "an ephemeral key generated per release"), so anyone with **release-asset-upload access alone** (no `github.com`/API compromise required) can mint a fresh keypair offline, sign a forged record with it, and upload record + signature + pubkey together as a self-consistent triple — this source's check alone would accept it. Its real value in that case is narrower: catching an asset **replaced inconsistently** with the others (e.g. only the record swapped, not the signature), not a **coherently forged set**. **`ManifestPubkeySource`** (opt-in, tried second when `--package` is given — jerus-org/jci-audit#124; resolved against the release repo's workspace `Cargo.toml` by package name, cargo's own `-p`/`--package` convention, rather than requiring a hand-typed path — but never runs unless the caller opts in with one) reads the pubkey from `Cargo.toml` at the release tag instead. **In jci-audit's own release pipeline, this is not an independently stronger anchor than the asset source.** `docs/RELEASING.md` step 5 and `.circleci/release.yml` show the same `toolkit/release_crate` CI job (context: `release`, `bot-check`, `pcu-app`) both injects the pubkey into `Cargo.toml`/pushes the tag *and* uploads the record and its signature as release assets — one job, one credential set. Compromising that job's credentials compromises both at once; there is no separately-operated second channel that also has to be broken. It additionally trusts a **mutable** git tag ref (`refs/tags/<tag>`), with no platform-enforced immutability, unlike the published-release-only guarantee GitHub Immutable Releases gives `AssetPubkeySource` — a force-moved tag silently changes what this source fetches. A genuinely independent, stronger anchor for jci-audit's own releases would require jci-audit's git-push and release-asset-upload credentials to actually be separate, which they are not today. |
...
- **GitHub API responses** (`verify`'s remote path) are deserialized into typed
  ...
  signing pubkey comes from one of two `PubkeySource`s, tried in order: the
  pubkey's own release asset (`release-<VERSION>.json.pub`), parsed for the
  bare key (tolerating either a plain key or the full `rsign`-generated
  pubkey-file format); or `Cargo.toml` at the release tag, parsed with
  `toml_edit` and only a known, fixed key path
  (`[package.metadata.binstall.signing].pubkey`) read from it.
- **`--owner`/`--repo`/`--tag-prefix`** (`verify`'s remote-fetch fallback,
  jerus-org/jci-audit#121) are plain strings passed as structured parameters
  to `pcu-release-assets`' typed API client, and interpolated into a
  `raw.githubusercontent.com` URL via `format!` for the manifest fetch —
  never through a shell, so there is no injection surface; a malformed value
  simply produces a URL that 404s or an API call GitHub rejects, not an
  unintended request elsewhere. **`--package`** (opt-in, jerus-org/jci-audit#124)
  is likewise a plain string, but never itself interpolated into a URL — it's
  compared, via `toml_edit`'s typed document model, against the `name` each
  fetched (and already-URL-safe) `Cargo.toml` path yields, not built into
  request paths itself.
...
- **jci-audit's own code now makes network calls, scoped to one path.**
  ...
  TLS via its own `reqwest`/`rustls` client) fetches the record and its
  signature as named release assets, and a direct `reqwest`/`rustls` HTTPS GET
  to `raw.githubusercontent.com` fetches the release tag's `Cargo.toml` when
  the caller supplies `--package` (see T9 for why this source is
  opt-in). Every other subcommand, and `verify` when a local
  record exists, makes no network calls of jci-audit's own — the only
  remaining transport-level touchpoint elsewhere is TLS on the advisory-db
  fetch, which is entirely delegated to `cargo-deny`'s own HTTPS client. See
  T9.
...
- **The release record is authenticated as a release asset.**
  ([#75](https://github.com/jerus-org/jci-audit/issues/75) phase 2) It is
  distributed alongside its own minisign signature and its own pubkey asset,
  which `verify`'s remote path checks first — a source that works for any
  consumer's release. jci-audit's own pubkey is additionally published in
  `Cargo.toml`, the same trust anchor `docs/RELEASING.md` documents for the
  binary tarball. See T6.
