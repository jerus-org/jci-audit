...
| **The GitHub token** (`verify`'s remote path, `GITHUB_TOKEN` env var only — optional, no CLI flag exists for it) | Trusted, caller-supplied, when present | Authenticates the fetch when set (raising GitHub's rate limit); an unset token falls back to the same requests unauthenticated (jerus-org/jci-audit#103) — the release and its assets are public. Held in memory only for the duration of the fetch; see T3. |
