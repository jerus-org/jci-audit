...
| **The GitHub token** (`verify`'s remote path, `GITHUB_TOKEN` env var only — optional for this public repo, no CLI flag exists for it) | Trusted, caller-supplied, when present | Authenticates the fetch when set; an unset token falls back to the same requests unauthenticated (jerus-org/jci-audit#103), which works because jerus-org/jci-audit is public — a private repo's release would need one, and the fetch would simply fail without it. Held in memory only for the duration of the fetch; see T3. |
...
| **T3** | **Credential leakage** — a signing key or token exposed in output, logs, or generated files. | `verify`'s remote fetch path is the only code path that ever reads a credential, and even there it's optional — for this project, because its repo is public: `GITHUB_TOKEN` **only** — deliberately not a CLI flag, since a secret passed as a command-line argument is visible to any other user on the same machine via `ps`/`/proc/<pid>/cmdline` (`cli.rs::run_verify_remote`). When set, it builds the `Authorization` header for `pcu-release-assets`' REST calls and `remote::ManifestPubkeySource`'s `reqwest` fetch; when unset, both go out unauthenticated instead (jerus-org/jci-audit#103), which works because jerus-org/jci-audit is public — a private repo's release would need a token, and the fetch would simply fail without one. Held in memory only, never written to a file or generated output, never logged. No other code path reads any credential. Satisfies R2. |
