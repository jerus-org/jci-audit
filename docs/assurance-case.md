...
| **T9** | **Man-in-the-middle on jci-audit's own new network calls** (`verify`'s remote path: the GitHub API via `pcu-release-assets`, and `raw.githubusercontent.com` for the manifest source), **and, separately, a forged release asset (or forged Cargo.toml).** | HTTPS (`reqwest` with `rustls`, TLS certificate verification on by default) mitigates transport-level tampering the same way T5 does for the advisory-db, on both legs. What the minisign check (T6) adds beyond that is **not the same for both pubkey sources, and neither is as independently strong as the ordering alone might suggest** — be honest about each: **`AssetPubkeySource`** (tried first — the only source with real data for *any* consumer's release, since every `jci-audit publish-record` invocation uploads one) has a materially weak property: minisign keys are free to generate (`docs/RELEASING.md` — "an ephemeral key generated per release"), so anyone with **release-asset-upload access alone** (no `github.com`/API compromise required) can mint a fresh keypair offline, sign a forged record with it, and upload record + signature + pubkey together as a self-consistent triple — this source's check alone would accept it. Its real value in that case is narrower: catching an asset **replaced inconsistently** with the others (e.g. only the record swapped, not the signature), not a **coherently forged set**. **`ManifestPubkeySource`** (a fallback tried only when the asset source fails — real data only for jci-audit's own releases, since [`MANIFEST_PATH`] is this repo's own workspace layout) reads the pubkey from `Cargo.toml` at the release tag instead. **This is not an independently stronger anchor for jci-audit's own releases.** `docs/RELEASING.md` step 5 and `.circleci/release.yml` show the same `toolkit/release_crate` CI job (context: `release`, `bot-check`, `pcu-app`) both injects the pubkey into `Cargo.toml`/pushes the tag *and* uploads the record and its signature as release assets — one job, one credential set. Compromising that job's credentials compromises both at once; there is no separately-operated second channel that also has to be broken. It additionally trusts a **mutable** git tag ref (`refs/tags/<tag>`), with no platform-enforced immutability, unlike the published-release-only guarantee GitHub Immutable Releases gives `AssetPubkeySource` — a force-moved tag silently changes what this source fetches. A third-party consumer using `jci-audit publish-record` has only the asset source available at all — the manifest fallback will simply fail closed for their repo and never apply. A genuinely stronger, independent anchor would require jci-audit's own git-push and release-asset-upload credentials to actually be separate, which they are not today. |
...
- **GitHub API responses** (`verify`'s remote path) are deserialized into typed
  ...
  pubkey-file format); or, as a jci-audit-specific fallback, `Cargo.toml` at
  the release tag, parsed with `toml_edit` and only a known, fixed key path
...
- **`--owner`/`--repo`/`--tag-prefix`** (`verify`'s remote-fetch fallback,
  ...
  to `pcu-release-assets`' typed API client and interpolated into a URL via
  `format!` for the manifest fallback — never through a shell, so there is no
  injection surface; a malformed value simply produces a URL that 404s or an
  API call GitHub rejects, not an unintended request elsewhere.
...
- **jci-audit's own code now makes network calls, scoped to one path.**
  ...
  the manifest pubkey source is tried, as a fallback after the asset source
  (see T9 for why). Every other subcommand, and `verify` when a local
...
- **The release record is authenticated differently than before, not less.**
  It is no longer signed as part of a git commit; instead
  ([#75](https://github.com/jerus-org/jci-audit/issues/75) phase 2) it is
  distributed as a release asset alongside its own minisign signature and its
  own pubkey asset, which `verify`'s remote path checks first — the source
  that works for any consumer's release, not only jci-audit's own (whose
  pubkey is additionally published in `Cargo.toml`, the same trust anchor
  `docs/RELEASING.md` documents for the binary tarball). See T6.
