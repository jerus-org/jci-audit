...
Options:
      --version-env <VERSION_ENV>  Env var NAME holding the version when VERSION is not given [default: SEMVER]
...
The version comes from the positional argument, or (for CI pipelines that compute the version at
runtime, e.g. via `nextsv`) from the environment variable named by `--version-env` (default
`SEMVER`) when it is omitted.
